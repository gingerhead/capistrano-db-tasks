module Asset
  ...
  def remote_to_local(cap)
    ...
    [cap.fetch(:assets_dir)].flatten.each do |dir|
      system("rsync -a --del -L -K -vv --progress --rsh='ssh -p #{port}' #{user}@#{server}:#{cap.shared_path}/#{dir} #{cap.fetch(:local_assets_dir)}")
    end
  end
  ...
  def local_to_remote(cap)
    ...
    user = server.netssh_options[:user]
    [cap.fetch(:assets_dir)].flatten.each do |dir|
      system("rsync -a --del -L -K -vv --progress --rsh='ssh -p #{port}' ./#{dir} #{user}@#{server}:#{cap.shared_path}/#{cap.fetch(:local_assets_dir)}")
    end
  end
